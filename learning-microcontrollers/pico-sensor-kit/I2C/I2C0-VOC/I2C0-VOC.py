...
class SGP40:
    def __init__(self,i2c_num=0,i2c_scl=9,i2c_sda=8,address=0x59):
        ...
        time.sleep(0.025)
        Rbuf = self.Read() 
        if ((((int(Rbuf[0]) << 8) | Rbuf[1]) != 0x3220) and (((int(Rbuf[0]) << 8) | Rbuf[1]) != 0x3240)):
    ...
    def Read(self):
        rdate = self.i2c.readfrom_mem(int(self.address), 0, 3)
        return rdate
    
    def write(self, cmd):
        self.i2c.writeto_mem(int(self.address), int(cmd[0]), bytes([int(cmd[1])]))
        
    def write_block(self, cmd):
        
        self.i2c.writeto_mem(int(self.address),int(cmd[0]),bytes(cmd[1:8]))
    ...
    def measureRaw(self, temperature, humidity):
        ...
        WITH_HUM_COMP[2:3] = list(paramh)
        WITH_HUM_COMP[4] = int(crch)
        WITH_HUM_COMP[5:6] = list(paramt)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            VOC.VocAlgorithm_process(raw)
            print("measureRaw Gas: ", raw)
            print("Voc:",VOC.VocAlgorithm_process(raw))
            time.sleep(0.3)
